  ...
  const finishDrawWall = useCallback(() => {
    ...
    if (currentPath.length >= 2) {
      ...
      if (
        ...
        const closedWall = createContinuousWall(closedPoints, true);
        if (closedWall) {
          canvas.add(closedWall);
          createPolygonWithPattern(closedPoints);
          updateWalls(closedWall, true);
        }
      } else {
        console.log("未形成封閉空間，保存為未完成牆體");
        const unfinishedWall = createContinuousWall(currentPath);
        if (unfinishedWall) {
          updateWalls(unfinishedWall, false);
        }
      }
    } else {
      console.log("點數不足，清除當前繪製");
      updateWalls(null, false);
    }
