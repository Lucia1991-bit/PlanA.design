...
export const useDrawWall = ({
  ...
  const updateWalls = useCallback(
    (newWall: fabric.Object | null) => {
      ...
      if (newWall) {
        //將新牆加入完成繪製的牆面，並移除重複的牆體
        setCompletedWalls((prev) => [
          ...prev.filter((w) => w !== newWall),
          newWall,
        ]);
      } else {
        // 當 newWall 為 null 時，清除未完成的牆體
        setCompletedWalls((prev) =>
  ...
  const startDrawWall = useCallback(() => {
    ...
    console.log("當前的未完成牆體:", unfinishedWall.current);
    ...
    [startPointRef, endPointRef, guideLineRef].forEach((ref) => {
      if (ref.current) {
        console.log(`正在移除 ${ref.current.name || "未命名物件"}`);
    ...
    console.log("重新渲染畫布並保存狀態");
    canvas?.renderAll();
    save();
  }, [setIsDrawingMode, unfinishedWall, canvas, save]);
  ...
  const finishDrawWall = useCallback(() => {
    ...
    const currentUnfinishedWall = unfinishedWall.current;
    if (currentUnfinishedWall && currentUnfinishedWall.get) {
      const allPoints = currentUnfinishedWall.get(
        "allPoints"
      ) as fabric.Point[];

      if (allPoints.length < 2) {
        console.log("點的數量不足，刪除當前點");
        canvas.remove(currentUnfinishedWall);
        updateWalls(null);
      } else {
        ...
        if (
          ...
          // 不需要再次調用 updateWalls，因為當前牆體已經是未完成狀態
        }
      }
    } else {
      console.log("沒有未完成的牆體，清除所有當前繪製狀態");
      updateWalls(null);
    }
