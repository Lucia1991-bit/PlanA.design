...
const Interactive500Canvas: React.FC = () => {
  ...
  const createAndCenterText = (canvas: fabric.Canvas) => {
    ...
    const text = new fabric.Text("500", {
      fontSize: 80, // 初始字體大小設置為較小的值
      fill: "#c6332e",
      fontWeight: "300",
  ...
  const updateTextSize = (canvas: fabric.Canvas) => {
    ...
    if (text) {
      const scaleFactor = Math.min(canvas.width!, canvas.height!) / 8; // 減小縮放因子
      const newFontSize = Math.max(scaleFactor, 24); // 降低最小字體大小
      text.set({
        fontSize: newFontSize,
        left: canvas.width! / 2,
        top: canvas.height! / 2,
      });
      text.setCoords(); // 更新物件的邊界框
    }
  };
...
export default Interactive500Canvas;
